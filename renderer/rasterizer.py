...
def calc_mv_mat(pivot, pos, rot_mat, view_mat):
...
@jit(nopython=True, fastmath=True)
def update_pts_data(screen_pts, z_list, w_list, uvs_list, tri_indices, px_py, z_vals, w_vals, uv_source, tri_cnt, tri_idx):
    ...
    uvs_list[tri_cnt, 0, 0] = uv_source[0, 0]
    uvs_list[tri_cnt, 0, 1] = uv_source[0, 1]
    uvs_list[tri_cnt, 1, 0] = uv_source[1, 0]
    uvs_list[tri_cnt, 1, 1] = uv_source[1, 1]
    uvs_list[tri_cnt, 2, 0] = uv_source[2, 0]
    uvs_list[tri_cnt, 2, 1] = uv_source[2, 1]

    tri_indices[tri_cnt] = tri_idx

@jit(nopython=True, fastmath=True)
def calc_view_tri(mesh, mv_mat, tri_view_list):
    tri_h = np.ones((3,4), np.float32)
    tri_t = np.zeros((3,4), np.float32)
    for tri_idx in prange(len(mesh)):
        tri = mesh[tri_idx]
        tri_view_list[tri_idx] = transform(tri, mv_mat, tri_h, tri_t)

@jit(nopython=True, fastmath=True)
def project_mesh(tri_view_list, uv, proj_mat, width, height, start_idx, end_idx, 
    ...
    for tri_idx in range(start_idx, end_idx):
        tri_view = tri_view_list[tri_idx]
        ...
        if calc_polygon(tri_proj_ndc, tri_proj_clip, px_py, z_vals, w_vals, width, height):
            ...
            loop_count = num_new_vertices // 3

        for i in range(loop_count):
            should_update = False
            
            if is_simple:
                uv_source = uv[tri_idx]
                should_update = True
            else:
                ...
                if calc_polygon(tri_proj_ndc, new_tri_clip, px_py, z_vals, w_vals, width, height):
                    uv_source = new_tri_uv
                    should_update = True
            
            if should_update:
                update_pts_data(screen_pts, z_list, w_list, uvs_list, tri_indices, px_py, z_vals, w_vals, uv_source, tri_cnt, tri_idx)
                tri_cnt += 1
...
def render_meshes_parallel(mesh_list, pivot_list, pos_list, rot_list, tex_list, uv_list, 
                        mesh_visible_flag, removed_flag, light_active_flag, light_data_list, diffuse_list,
                        view_mat, proj_mat, width, height, 
                        vertices_buffer, framebuffer, z_buffer, object_buffer, 
    ...
    tri_view_list = np.empty((MAX_TRIS, 3, 3), dtype=np.float32)
    ...
    for mesh_idx in range(len(mesh_list)):
        ...
        rot_mat = rz @ ry @ rx
        mv_mat = calc_mv_mat(pivot, pos, rot_mat, view_mat)
        calc_view_tri(mesh, mv_mat, tri_view_list)
        for band_index in prange(NUM_BANDS):
            ...
            tri_cnt = project_mesh(tri_view_list, uvs, proj_mat, width, height, start_idx, end_idx, screen_pts, z_list, w_list, uvs_list, tri_indices)
...
def render_meshes_serial(mesh_list, pivot_list, pos_list, rot_list, tex_list, uv_list, 
                        mesh_visible_flag, removed_flag, light_active_flag, light_data_list, diffuse_list,
                        view_mat, proj_mat, width, height, 
                        vertices_buffer, framebuffer, z_buffer, object_buffer, mode):
    vertex_count = 0
    pixels_drawn = 0
    max_tris = 0
    ...
    for mesh_idx in range(len(mesh_list)):
        ...
        mv_mat = calc_mv_mat(pivot, pos, rot_mat, view_mat)
        calc_view_tri(mesh, mv_mat, tri_view_list)
        tri_cnt = project_mesh(tri_view_list, uvs, proj_mat, width, height, 0, len(mesh), screen_pts, z_list, w_list, uvs_list, tri_indices)
        for j in range(tri_cnt):
            pts = screen_pts[j]
            z_vals = z_list[j]
            w_vals = w_list[j]
...
def render_meshes(mesh_list, pivot_list, pos_list, rot_list, tex_list, uv_list, mesh_visible_flag, removed_flag, light_active_flag, light_data_list, diffuse_list, view_mat, proj_mat, width, height, vertices_buffer, framebuffer, z_buffer, object_buffer, thread_buffers, parallel, mode):
    if parallel: 
        return render_meshes_parallel(mesh_list, pivot_list, pos_list, rot_list, tex_list, uv_list, mesh_visible_flag, removed_flag, light_active_flag, light_data_list, diffuse_list, view_mat, proj_mat, width, height, vertices_buffer, framebuffer, z_buffer, object_buffer, *thread_buffers, mode)
    else: 
        return render_meshes_serial(mesh_list, pivot_list, pos_list, rot_list, tex_list, uv_list, mesh_visible_flag, removed_flag, light_active_flag, light_data_list, diffuse_list, view_mat, proj_mat, width, height, vertices_buffer, framebuffer, z_buffer, object_buffer, mode)